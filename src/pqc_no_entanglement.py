"""
PQC No-Entanglement Ablation Variant
=====================================

This module is an ablation variant of ``pqc_compressor.py`` with **all CNOT
entanglement gates removed** from the variational quantum circuit.

Purpose:
    Isolate the effect of inter-qubit entanglement on barren plateaus.
    By comparing the training dynamics and gradient behaviour of this
    variant against the full PQC (which uses a CNOT ring), we can
    quantify how much entanglement contributes to vanishing gradients.

Circuit structure (per variational layer):
    1. Angle encoding: RY(input[i]) on each qubit
    2. Single-qubit rotations RY, RZ  (NO CNOT ring)
    3. Measurement: ⟨Z_i⟩ for each qubit

Everything else — training loop, stagnation detection, class-weighted
loss, hyperparameters — is kept **identical** to the entangled version
so that any performance delta is attributable solely to the removal of
entanglement.
"""
...
def create_qnode(n_qubits: int, n_layers: int):
    ...
    def circuit(inputs, weights):
        ...
        # Variational layers — single-qubit rotations ONLY, no CNOT
...
class PQCNoEntanglementModel(nn.Module):
    """
    Hybrid classical-quantum model for embedding compression
    **without** entanglement gates (ablation variant).

    Architecture:
        1. Linear projection: 768 → n_qubits (with tanh to bound inputs to [-1,1] → scale to [-π,π])
        2. Quantum circuit: angle encoding + variational layers (no CNOT) → n_qubits expectation values
...
class PQCNoEntanglementCompressor:
    """Wrapper for training and using the PQC hybrid model without entanglement."""

    def __init__(self, d_out: int):
        self.d_out = d_out
        self.model = PQCNoEntanglementModel(
    ...
    def fit(
        ...
        """Train PQC (no entanglement) end-to-end with class-weighted cross-entropy loss."""
        ...
        print(f"  Training PQC-NoEntanglement (d_out={self.d_out})...")
        ...
        for epoch in range(config.PQC_EPOCHS):
            ...
            if val_loss < best_val_loss:
                ...
                if stagnation_counter == STAGNATION_THRESHOLD:
                    msg = (f"PQC-NoEntanglement gradient stagnation: val_loss did not improve for "
        ...
        print(f"  PQC-NoEntanglement training complete. Final val_acc: {val_acc:.4f}")
        if self.anomalies:
            print(f"  [WARNING] {len(self.anomalies)} stagnation warning(s) recorded.")

    def transform(self, embeddings: torch.Tensor) -> np.ndarray:
        """Extract compressed embeddings from trained PQC (no entanglement)."""
        self.model.eval()
        all_compressed = []
    ...
    def predict(self, embeddings: torch.Tensor) -> np.ndarray:
        """Predict class labels using the full hybrid model (no entanglement)."""
        self.model.eval()
        all_preds = []
        dataset = TensorDataset(embeddings)
        loader = DataLoader(dataset, batch_size=config.PQC_BATCH_SIZE, shuffle=False)
